...
// Initial locations data for auto-seeding
const INITIAL_LOCATIONS = [
  ...
  { name: "Porte de Versailles - Entrepôt", address: "1 Place de la Porte de Versailles, Paris", lat: 48.8323, lng: 2.2876, type: LocationType.INDUSTRIAL, district: "Paris 15ème", rentPerSqm: 40, footTraffic: 30, visibility: 40, accessibility: 70, isActive: true },
  // Paris - Aéroport
  ...
  { name: "Vaise - Zone Industrielle", address: "Quartier de Vaise, Lyon", lat: 45.7700, lng: 4.8000, type: LocationType.INDUSTRIAL, district: "Lyon 9ème", rentPerSqm: 35, footTraffic: 25, visibility: 35, accessibility: 60 },
  // Marseille
  ...
  { name: "Grand Littoral - Zone Portuaire", address: "Port de Marseille", lat: 43.3500, lng: 5.3500, type: LocationType.PORT, district: "Marseille Port", rentPerSqm: 55, footTraffic: 40, visibility: 50, accessibility: 70 },
  // Bordeaux
...
export async function GET(req: Request) {
  ...
  try {
    // Check if we need to seed locations
    const existingCount = await db.gameLocation.count()
    if (existingCount === 0) {
      // Auto-seed initial locations
      for (const loc of INITIAL_LOCATIONS) {
        await db.gameLocation.create({ data: loc as any })
      }
    }
